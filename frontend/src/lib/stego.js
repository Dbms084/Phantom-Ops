...
export async function sendHighSensitivityACK(messageId, projectId) {
  const response = await fetch(`${API_URL}/messages/${messageId}/destroy`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error('Failed to send High Sensitivity ACK');
  }
